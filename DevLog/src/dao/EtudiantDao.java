...
public class EtudiantDao extends ConnectionDao{
	...
	public boolean isStudent (Student etu) {
			...
			try {
				...
				while(rs.next()){
					listEtudiant.add(new Student(rs.getInt("IDETUDIANT"), rs.getString("NOM"), rs.getString("PRENOM"),
														rs.getString("DATEDENAISSANCE"), rs.getInt("CLASSEMENT"), null, rs.getInt("PROMOTION"), rs.getInt("FILIERE")));
				}
		...
		public int update(Student etu) {
			...
			try {
				con = DriverManager.getConnection(URL, LOGIN, PASS);
				ps = con.prepareStatement("UPDATE STUDENT SET NOM = ?, PRENOM = ?, DATEDENAISSANCE = ?, CLASSEMENT = ? , PROMOTION = ? , FILIERE = ? WHERE IDETUDIANT = ?");
